...
namespace API.SignalR
{
    ...
    {
        private static readonly Dictionary<string, List<string>> OnlineUsers = new Dictionary<string, List<string>>();   // Dictionary has key/value pairs, with string username as Key/ List<string> value connection ID as string
       ...
       // public Task UserConnected(string username, string connectionId)
        public Task<bool> UserConnected(string username, string connectionId)
        {
            ...
            lock (OnlineUsers)                                         // the approach is to lock the user until we finish using the dictionary   // not a thread safe resource
            {
       ...
       // public Task UserDisconnected(string username, string connectionId)
        ...
        public Task<string[]> GetOnlineUsers()                    // this is our method to get the users currently online  // we return an array of users
        {
            ...
            {
                onlineUsers = OnlineUsers.OrderBy(k => k.Key).Select(k => k.Key).ToArray();         // stored in memory not database
            }
