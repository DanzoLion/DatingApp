...
namespace API.Data
{
    public class MessageRepository : IMessageRepository
    {
        ...
        public async Task<Connection> GetConnection(string connectionId) {return await _context.Connections.FindAsync(connectionId);}
        ...
        public async Task<PagedList<MessageDto>> GetMessagesForUser(MessageParams messageParams)   // added MessageParms messageParams here
        {
            ...
            var query = _context.Messages.OrderByDescending(m => m.MessageSent).ProjectTo<MessageDto>(_mapper.ConfigurationProvider).AsQueryable();     // we projectTo Message Dto
            query = messageParams.Container switch
            {
                "Inbox" => query.Where(u => u.RecipientUsername == messageParams.Username && u.RecipientDeleted == false),            // this serves as our inbox // if we are recipient and read it this is what goes back from here
                "Outbox" => query.Where(u => u.SenderUsername == messageParams.Username && u.SenderDeleted == false),            // this serves as our outbox // if we are recipient and read it this is what goes back from here
                _ => query.Where(u => u.RecipientUsername == messageParams.Username && u.RecipientDeleted == false && u.DateRead == null)            // this serves as our default option within the switch statement
            };
            // from here we want to project and return DTOs from this here // we need to bring IMapper into this repository also
         //   var messages = query.ProjectTo<MessageDto>(_mapper.ConfigurationProvider);     // we projectTo Message Dto  // removed with messaging re-factor 
            return await PagedList<MessageDto>.CreateAsync(query, messageParams.PageNumber, messageParams.PageSize);  // messages replaced with query for message refactor clean
        }
        ...
        public async Task<IEnumerable<MessageDto>> GetMessageThread(string currentUsername, string recipientUsername)           // here we get the message thread between two users, ie the conversation
        {
            ...
            // .Include(u => u.Recipient).ThenInclude(p => p.Photos)       // displays users photo in the message design also           // we don't need include statements when we use projection
        ...
        var unreadMessages = messages.Where(m => m.DateRead == null && m.RecipientUsername == currentUsername).ToList();  // we end up looping over this object as a list // get any unread messages // changed to RecipientUsername from Recipient.UserName
...
//            await _context.SaveChangesAsync();
        ...
        // public async Task<bool> SaveAllAsync()                                             // removed with UnitOfWork
        // {
