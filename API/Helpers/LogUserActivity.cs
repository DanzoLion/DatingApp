...
namespace API.Helpers
{
    public class LogUserActivity : IAsyncActionFilter
    {
        ...
        //    var repo = resultContext.HttpContext.RequestServices.GetService<IUserRepository>();   // accesses our repository
            var uow = resultContext.HttpContext.RequestServices.GetService<IUnitOfWork>();   // accesses our repository
           //  var user = await repo.GetUserByUsernameAsync(username);
            var user = await uow.UserRepository.GetUserByIdAsync(userId); // updated to reflect extension and services changes
            user.LastActive = DateTime.UtcNow;       // updated from Now -> UtcNow
