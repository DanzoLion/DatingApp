...
namespace API.Data
{
    public class UserRepository : IUserRepository // implement interface from IUserRepository for AutoFill
    {
        ...
        // public async Task<bool> SaveAllAsync()                                            // removed with UnitOfWor
        // {
        ...
        {
            return await _context.Users.Where(x => x.UserName == username).Select(x => x.Gender).FirstOrDefaultAsync();    // returns a single property of gender from our database when the method is implemented
        }
    }
}