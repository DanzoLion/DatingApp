...
namespace API.Controllers {
    ...
    public class UsersController : BaseApiController                                                                                                                          // first derive from ControllerBase   // dependency injection will be used to get data from database here // modified: to BaseApiController
    {
        ...
        // private readonly IUserRepository _unitOfWork.UserRepository;                      // removed with UnitOfWork                                  // inserted after UsersController updated
       ...
       // public UsersController(IUserRepository userRepository, IMapper mapper, IPhotoService photoService)                                         // inject IUserRepository Interface after we created it and call it userRepository // added IPhotoService as we are adding our Photo service below
        public UsersController(IUnitOfWork unitOfWork, IMapper mapper, IPhotoService photoService)                                         // inject IUserRepository Interface after we created it and call it userRepository // added IPhotoService as we are adding our Photo service below
        {
            ...
            // _unitOfWork.UserRepository = userRepository;                                                                                                                                          // created after updated IUserRepository 
      ...
      //  [Authorize(Roles = "Admin")]          // only Admin can get a full list of users here
        ...
        {                                                                                                                                                                                                // async added for asynchronous threading // then wrap ActionResult<> into a task for asynchronous threading
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        //var users = await _unitOfWork.UserRepository.GetUserAsync();               // first deposit await into users variable    // changed to single line wrapper below
                                                                                                                                                                                                      //  return Ok(await _unitOfWork.UserRepository.GetUserAsync());                                              // return users;   // we return users from endpoint // change .ToList() to ToListAsync() - for asynchronous threading    // changed to return Ok(users);
                                                                                                                                                                                                      // var users = await _unitOfWork.UserRepository.GetUserAsync();          **************** changed upon new implementation of new AutoMapper implementation ********************
        ...
        var gender = await _unitOfWork.UserRepository.GetUserGender(User.GetUsername());   // we can implement GetUserGender() once we create the method to retun the single property Gender
       // userParams.CurrentUsername = user.UserName;            // get username of user; then add a check for their gender to set default gender
        userParams.CurrentUsername = User.GetUsername();            // User.GetUsernam() retrieves a less complex query from out database and simplifies the output to our api server
         if (string.IsNullOrEmpty(userParams.Gender))
         //userParams.Gender = user.Gender == "male" ? "female" : "male";            // checks if user is male or femal, if female set to male 
         userParams.Gender = gender == "male" ? "female" : "male";            // replaced with .gender after creating the method to return property gender 
         var users = await _unitOfWork.UserRepository.GetMembersAsync(userParams);                                                             // we are then able to pass userParms here once it's implemented above
        ...
        return Ok(users);
                                                                                                                                                  //  return Ok(usersToReturn);                                         // cut here  await _unitOfWork.UserRepository.GetUserAsync() and placed int var users  // changed after new AutoMapper implementation
     ...
     //  [Authorize(Roles = "Member")]                        // Member can get an individual user
       [HttpGet("{username}", Name = "GetUser")]                                                                // we add Name = GetUser as a root name for CreatedAtRoute()
        ...
        {                                                                                                                                                                   // return user; removed as we don't need to specify a variable  // we return users from endpoint // added async Task<> & await .FindAsync
                                                                                                                                                                          ...
                                                                                                                                                                          //  return await _unitOfWork.UserRepository.GetUserByUsernameAsync(username);          // we now get the user via username instead of ID   // re-written to handle Dto

                                                                                                                                                                                   //var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(username);      // AutoMember no takes care of automapping between our AppUser and MemberDto // changed when implementing new _mapper in UserRepository.cs
       return await _unitOfWork.UserRepository.GetMemberAsync(username);                                                                                 // AutoMember no takes care of automapping between our AppUser and MemberDto
      ...
      public async Task<ActionResult> UpdateUser(MemberUpdateDto memberUpdateDto)                                                     // our method // returns action result // don't need to send data back as client has all data we have for entity we are creating here
      {
           ...
           var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername());                                                   // once we have user we use the user repository and user by user async
           
           _mapper.Map(memberUpdateDto, user);                                                                                                            // we map the user to Dto here
           _unitOfWork.UserRepository.Update(user);                                                                                                                         // here the user object is flagged as being updated by entity framework // guarantees we don't get an error or exception

         //  if (await _unitOfWork.UserRepository.SaveAllAsync()) return NoContent();                 // removed wiht UnitOfWrok      // no content sent back here and saves if successful
           if (await _unitOfWork.Complete()) return NoContent();                                                                           // no content sent back here and saves if successful

           return BadRequest("Failed to update user");                                                                                                  // if not _unitOfWork.UserRepository .. return BadRequest
      }
    ...
    {
        var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername());                                       // user object also converted to a one-liner 
      ...
      if (await _unitOfWork.Complete())                                                                                                                    // if true save all photo's
      {
  ...
  {                                                                                                               // server has signed token, user has sent the token, and we can trust this is correct, the user then authenticates to the method below
    ...
    var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername());  // here we get our user by their username // when we get user from token as in this case we say the user is who they say they are, ie no validation necessary here
    // FirstOrDefault() is not async as we already have user in memory at this point, ie extracted from _unitOfWork.UserRepository,  x => x.Id looks for the photo we get from the parameter photoId
    ...
    if (await _unitOfWork.Complete()) return NoContent();                   // next we need to save our changes back to the _unitOfWork.UserRepository //  .. return NoContent() as we don't need to send anything back
...
{
  var user = await _unitOfWork.UserRepository.GetUserByUsernameAsync(User.GetUsername());          // first we need to get our user object  // we retrieve the user by username // then below identify phot we are interested in only
  ...
  if (await _unitOfWork.Complete()) return Ok();        // if complete we return simple Ok // if not successful saving this return bad request below
